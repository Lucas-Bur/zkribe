...
export const compressAudioToOptimizedMp3 = async (audioFile: File): Promise<File> => {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        const newFileName = audioFile.name.replace(`.${extension}`, "_optimized.mp3")
        const convertedFile = new File([mp3Blob], newFileName, {
          type: "audio/mp3",
        })
        resolve(convertedFile)
      } catch (error) {
        console.error("❌ Error compressing existing audio to optimized MP3:", error)
        reject(error)
      }
    }
